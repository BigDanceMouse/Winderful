...
struct WeatherNetworkServiceImp: WeatherNetworkService {
    ...
    func requestWeather(for location: Coordinates, then completion: @escaping (WeatherResult) -> Void) {
        guard let url = WeatherRequestURL(for: location, token: token) else {
            completion(.failure(.incorrectRequestParametrs))
            return
        }
...
private func WeatherRequestURL(for coordinates: Coordinates, token: String) -> URL? {
    struct Constants {
